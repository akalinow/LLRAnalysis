...
void synchNtuple(string sample = "Embedded_2012D_LowPt", string stream = "MuTau", TString path="", TString file="", bool incl=false) {

  cout << "Start producing sync ntuple for sample : " << sample << " (" << stream << ")" << endl;

  TFile *lOFile = new TFile(("Sync_mts_LLR_"+sample+".root").c_str(),"RECREATE");
  TTree *lOTree = new TTree("TauCheck","TauCheck");
   ...
   // Soft analysis //
   float lETM         = 0; lOTree->Branch("l1met"        ,&lETM           ,"l1met/F"      );   // L1 ETM
   float lETMcorr     = 0; lOTree->Branch("l1metcorr"    ,&lETMcorr           ,"l1metcorr/F"      );   // L1 ETMcorr

   float lETMpass     = 0; lOTree->Branch("l1metpass"    ,&lETMpass       ,"l1metpass/F"  );   // passL1etmCut
   float lETMpassUp   = 0; lOTree->Branch("l1metpassUp"  ,&lETMpassUp     ,"l1metpassUp/F");   // passL1etmCutUp
   float lETMpassDown = 0; lOTree->Branch("l1metpassDown",&lETMpassDown   ,"l1metpassDown/F"); // passL1etmCutDown

   float lCaloMet     = 0; lOTree->Branch("calomet"      ,&lCaloMet       ,"calomet/F"    );   // caloMEtNoHF
   float lCaloMetUp   = 0; lOTree->Branch("calometUp"    ,&lCaloMetUp     ,"calometUp/F"    );   // caloMEtNoHFUp
   float lCaloMetDown = 0; lOTree->Branch("calometDown"  ,&lCaloMetDown   ,"calometDown/F"    );   // caloMEtNoHFDown

   float lCaloMetcorr     = 0; lOTree->Branch("calometcorr"      ,&lCaloMetcorr       ,"calometcorr/F"    );   // caloMEtNoHF
   float lCaloMetcorrUp   = 0; lOTree->Branch("calometcorrUp"    ,&lCaloMetcorrUp     ,"calometcorrUp/F"    );   // caloMEtNoHFUp
   float lCaloMetcorrDown = 0; lOTree->Branch("calometcorrDown"  ,&lCaloMetcorrDown   ,"calometcorrDown/F"    );   // caloMEtNoHFDown
   ...
   int   lHLTmatchMC  = 0; lOTree->Branch("HLTmatchMC"  ,&lHLTmatchMC ,  "HLTmatchMC/I");
   ...
   float iHLTx          ; tree->SetBranchAddress("HLTxSoft"            ,&iHLTx   );  // soft analysis (Data trigger firing)
   float iHLTmatch      ; tree->SetBranchAddress("HLTmatchSoft"        ,&iHLTmatch); // soft analysis (Data trigger matching)
   float iHLTmatchMC    ; tree->SetBranchAddress("HLTmatchIsoMu8Tau20" ,&iHLTmatchMC); // soft analysis (MC   trigger matching)
   ...
   float iPUWeightLow  ; tree->SetBranchAddress("puWeightDLow"   ,&iPUWeightLow  );//Pielup Weight D Low  (soft)
   float iPUWeightHigh ; tree->SetBranchAddress("puWeightDHigh"  ,&iPUWeightHigh );//Pielup Weight D High (soft)
   ...
   float iETM         = 0; 
   float iETMcorr     = 0; 
   if(sample.find("Emb") != string::npos)  tree->SetBranchAddress("L1etm"        ,&iETMcorr );   //L1 ETM
   else                                    tree->SetBranchAddress("L1etmCorr"    ,&iETMcorr );   //L1 ETM
   tree->SetBranchAddress("L1etm"    ,&iETM );   //L1 ETM     

   if(DEBUG) cout << "Get passL1etmCut branches" << endl;
   float iETMpass     = 0; tree->SetBranchAddress("passL1etmCut"    ,&iETMpass     );   // passL1etmCut
   float iETMpassUp   = 0; tree->SetBranchAddress("passL1etmCutUp"  ,&iETMpassUp   );   // passL1etmCutUp
   float iETMpassDown = 0; tree->SetBranchAddress("passL1etmCutDown",&iETMpassDown );   // passL1etmCutDown

   // CaloMet
   float iCaloMet       = 0; 
   float iCaloMetUp     = 0;
   float iCaloMetDown   = 0; 

   float iCaloMetcorr       = 0; 
   float iCaloMetcorrUp     = 0;
   float iCaloMetcorrDown   = 0; 

   if(DEBUG) cout << "Get caloMEtNoHF branches" << endl;
   if(sample.find("Data") != string::npos) {
     tree->SetBranchAddress("caloMEtNoHFUncorr"  ,&iCaloMetcorr     );// caloMEtNoHF
     tree->SetBranchAddress("caloMEtNoHFUncorr"  ,&iCaloMetcorrUp   );// caloMEtNoHFUp
     tree->SetBranchAddress("caloMEtNoHFUncorr"  ,&iCaloMetcorrDown );// caloMEtNoHFDown
   }
   else {
     tree->SetBranchAddress("caloMEtNoHF"      ,&iCaloMetcorr     );// caloMEtNoHF
     tree->SetBranchAddress("caloMEtNoHFUp"    ,&iCaloMetcorrUp   );// caloMEtNoHFUp
     tree->SetBranchAddress("caloMEtNoHFDown"  ,&iCaloMetcorrDown );// caloMEtNoHFDown
   }
   tree->SetBranchAddress("caloMEtNoHFUncorr"  ,&iCaloMet     );// caloMEtNoHF
   tree->SetBranchAddress("caloMEtNoHFUncorr"  ,&iCaloMetUp   );// caloMEtNoHFUp
   tree->SetBranchAddress("caloMEtNoHFUncorr"  ,&iCaloMetDown );// caloMEtNoHFDown
   
   float iHLTweightTau ; tree->SetBranchAddress("HLTweightTauD", &iHLTweightTau);
   float iHLTTau ;       tree->SetBranchAddress("HLTTauD", &iHLTTau);
   float iSFTau   ;      tree->SetBranchAddress("SFTau", &iSFTau);
   float iHLTweightMu, iSFMu, iSFMuID, iSFMuIso;
   float iHLTMu;
   if(stream.find("MuTau")!=string::npos ) {
     tree->SetBranchAddress("HLTweightMuSoft", &iHLTweightMu); 
     tree->SetBranchAddress("HLTMuSoft", &iHLTMu);
     tree->SetBranchAddress("SFMu_D", &iSFMu);
     tree->SetBranchAddress("SFMuID_D", &iSFMuID);
     tree->SetBranchAddress("SFMuIso_D", &iSFMuIso);
   }
   ...
   if(stream.find("ElecTau")!=string::npos ) {
     ...
     tree->SetBranchAddress("nHits"       ,&inHits );//
   }
   ...
   int itightestAntiMuWP         ; tree->SetBranchAddress("tightestAntiMuWP"       ,&itightestAntiMuWP    );
   ...
   // Produce skim
   TCut selMu     = "ptL1>9 && ptL1<=20  && isPFMuon && isTightMuon && combRelIsoLeg1DBetav2<0.10" ;
   TCut selTau    = "ptL2>20 && hpsDB3H<1.5 && tightestAntiMuWP>2" ;
   TCut vetoEvent = "vetoEventOld==0 && muFlag!=1" ; 
   TCut selection = selMu && selTau && vetoEvent;
   //TCut selection = selMu && selTau ;

   if(sample.find("Data") != string::npos) selection= selection && "HLTxSoft && HLTmatchSoft" ;
   if(sample.find("Emb") != string::npos)  selection = selection && "genDiTauMass>50" ; 

   //if(choosePairOS) selection = selection && "diTauCharge==0" ;

   //else if(sample.find("Emb")  != string::npos) selection= selection;
   //else                                         selection= selection && "HLTxSoft && HLTmatchIsoMu8Tau20" ;
   ...
   tree->SetBranchStatus("tightestAntiMuWP",  1);   
   tree->SetBranchStatus("vetoEventOld",  1);   
   tree->SetBranchStatus("muFlag",  1);   
   tree->SetBranchStatus("HLTxSoft",  1);   
   tree->SetBranchStatus("HLTmatchSoft",  1);   
   tree->SetBranchStatus("HLTmatchIsoMu8Tau20",  1);   
   ...
   for (Long64_t i=0; i<nPairs;i++) {
     ...
     //if(incl && !(iDiTauCharge == 0 && iMt1 < 30)) continue; //inclusive selection (on top of the skim)
     if(incl && !(iDiTauCharge == 0)) continue; 
     ...
     lPUWeight    = iPUWeightLow*iPUWeightHigh;
     if(sample.find("Emb") != string::npos){
       lEffWeight   = stream.find("MuTau")!=string::npos ? iHLTTau*iHLTMu*iSFMuID : iHLTTau*iHLTElec*iSFElecID;
     }
     else {
       lEffWeight   = stream.find("MuTau")!=string::npos ? iHLTTau*iHLTMu*iSFTau*iSFMu : iHLTTau*iHLTElec*iSFTau*iSFElec;
     ...
     lTrigweight_1 = stream.find("MuTau")!=string::npos ? iHLTMu : iHLTElec;
     lTrigweight_2 = iHLTTau;
     ...
     */
     ...
     lETMcorr     = iETMcorr;
     lETMpass     = iETMpass;
     lETMpassUp   = iETMpassUp;
     lETMpassDown = iETMpassDown;

     // CaloMet
     lCaloMet     = iCaloMet;
     lCaloMetUp   = iCaloMetUp;
     lCaloMetDown = iCaloMetDown;

     lCaloMetcorr     = iCaloMetcorr;
     lCaloMetcorrUp   = iCaloMetcorrUp;
     lCaloMetcorrDown = iCaloMetcorrDown;
     ...
     if(DEBUG && i%500==0) cout << "SVfit mass = " << iMSV << endl;
     ...
     lagainstMuonLoose2 = (itightestAntiMuWP > 0) ? 1.0 : 0.;
     lagainstMuonMedium2 = (itightestAntiMuWP > 1) ? 1.0 : 0.;
     lagainstMuonTight2 = (itightestAntiMuWP > 2) ? 1.0 : 0.;
...
void synchNtupleAll()
{
  ...
  //TString path="/data_CMS/cms/htautau/PostMoriond/NTUPLES_Summer13_NoTES/MuTau/";
  TString path="/data_CMS/cms/htautau/PostMoriond/NTUPLES_Summer13_TES/MuTau/update6/";

  synchNtuple("Embedded_2012D_LowPt", "MuTau", path, "nTupleRun2012D-22Jan2013-p*-EmbeddedLowPt_MuTau_nominal.root", true);
  //synchNtuple("Data_2012D"          , "MuTau", path, "nTupleRun2012D-22Jan2013-p*-Data_MuTau.root", true);
  //synchNtuple("VBFH125"             , "MuTau", path, "nTupleVBFH125_MuTau_nominal.root", true);
  //synchNtuple("DYTauTau"            , "MuTau", path, "nTupleDYJets_TauTau_MuTau_nominal.root", false);
