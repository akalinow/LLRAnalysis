...
def treeSkim( ana, sample, runInSeries=False):
    ...
    if runInSeries:
         ...
         configFileName = "./Configs/treeSkimmerElecTau_Fall14_%s_%s_cfg.py" % (sample,ana)
         runInSeries   = 'treeSkimmer'+stream+'_Fall14 '+configFileName+'\n'
         print runInSeries
        ...
        ##
        configFileName = "./Configs/treeSkimmerElecTau_Fall14_%s_%s_cfg.py" % (sample,ana)
        f.write('treeSkimmer'+stream+'_Fall14 '+configFileName+' > '+fileLog+'\n')
        #f.write('treeSkimmer'+stream+'_Spring14 '+configFileName+' > '+fileLog+'\n')
        f.close()
        os.system('chmod u+x batch/*.sh')

os.system('python makeTreeSkimmerElecTau_Fall14.py')
...
ListFinalState = ['TauTau','JetToTau','EToTau','ZTTJ']
...
##    treeSkim( anaMC[iAnMC] , "TTJets"              , False)
    ...
    treeSkim( anaMC[iAnMC] , "TTJets_SemiLept_p0", False)
    treeSkim( anaMC[iAnMC] , "TTJets_SemiLept_p1", False)
    treeSkim( anaMC[iAnMC] , "TTJets_SemiLept_p2", False)
    treeSkim( anaMC[iAnMC] , "TTJets_SemiLept_p3", False)
    treeSkim( anaMC[iAnMC] , "TTJets_SemiLept_p4", False)
    treeSkim( anaMC[iAnMC] , "TTJets_SemiLept_p5", False)
    treeSkim( anaMC[iAnMC] , "TTJets_SemiLept_p6", False)
    treeSkim( anaMC[iAnMC] , "TTJets_SemiLept_p7", False)
    treeSkim( anaMC[iAnMC] , "TTJets_SemiLept_p8", False)
    treeSkim( anaMC[iAnMC] , "TTJets_SemiLept_p9", False)
