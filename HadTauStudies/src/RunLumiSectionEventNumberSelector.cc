...
RunLumiSectionEventNumberSelector::RunLumiSectionEventNumberSelector(const edm::ParameterSet& cfg)
{
  //std::cout << "<RunLumiSectionEventNumberSelector::RunLumiSectionEventNumberSelector>:" << std::endl;
  ...
  if ( inputFileName_ == "" ) {
    std::cerr << "<RunLumiSectionEventNumberSelector::RunLumiSectionEventNumberFilter>: Invalid Configuration Parameter 'inputFileName' = " << inputFileName_ << " !!";
    assert(0);
  }
  readInputFile();
...
RunLumiSectionEventNumberSelector::~RunLumiSectionEventNumberSelector()
{
  std::string matchRemark = ( numEventsSelected_ == numEventsToBeSelected_ ) ? "matches" : "does NOT match";
  std::cout << "<RunLumiSectionEventNumberSelector::~RunLumiSectionEventNumberSelector>:" 
...
void RunLumiSectionEventNumberSelector::readInputFile()
{
//--- read run + luminosity section + event number pairs from ASCII file
  ...
  ifstream inputFile(inputFileName_.data());
  int iLine = 0;
...
//    and match individually run, event and luminosity section numbers
    ...
    if ( parseError ) {
      std::cerr << "<RunLumiSectionEventNumberSelector::readInputFile>: Error in parsing line " << iLine << " = '" << line << "'" << " of input file = " << inputFileName_ << " !!" << std::endl;
      assert(0);
    }
  }

  if ( numEventsToBeSelected_ == 0 ) {
    std::cerr << "<RunLumiSectionEventNumberSelector::readInputFile>: Failed to read any run+ls+event numbers from input file = " << inputFileName_ << " !!" << std::endl;
    assert(0);
  }
}

bool RunLumiSectionEventNumberSelector::operator()(ULong_t run, ULong_t ls, ULong_t event) const
{
//--- check if run number matches any of the runs containing events to be selected
  ...
  ++numEventsProcessed_;
  if ( isSelected ) {
    std::cout << "<RunLumiSectionEventNumberSelector::operator>: selecting run# = " << run << ", ls# " << ls << ", event# " << event << std::endl;
