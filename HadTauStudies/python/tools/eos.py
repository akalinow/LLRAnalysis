...
def chmod(path, value):
    '''
    Set access permission for EOS directory.
    An exception of type RuntimeError will be raised in case directory does not exist.
    '''
    try:
        lsl(path)        
    except IOError:
        raise RuntimeError("Path %s does not exist !!" % path)
    runCommand('%s chmod %i %s' % (executable_eos, value, path))
