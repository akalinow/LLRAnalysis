...
def getReportForSample(path,skimType,crabDirPattern=""):
    ...
    for crabDir in os.listdir(path):
        ...
        ##############            
            ...
            dbsDatasetName="/"+datasetpath.split("/")[1]+"/ndaci-"+publish_data_name
            print "DBS Instance: ",dbsInst
            print "DBS Dataset name: ",dbsDatasetName
            print "DBS Link: ","https://cmsweb.cern.ch/dbs_discovery/aSearch?caseSensitive=on&userMode=user&sortOrder=desc&sortName=&grid=1&method=dbsapi&dbsInst="+dbsInst+"&userInput=find+dataset+where+dataset+like+*"+dbsDatasetName+"*"+skimType+"*/USER+and+dataset.status+like+VALID*"
            #print "DAS Link: ","https://cmsweb.cern.ch/das/request?view=list&limit=10&instance="+dbsInst+"&input=dataset%3D"+dbsDatasetName.replace("/","%2F")+"*"+skimType+"*/USER"
            print "DAS Link: ","https://cmsweb.cern.ch/das/request?view=list&limit=10&instance="+dbsInst+"&input=dataset%3D"+publishName.replace("/","%2F")+"]["+publishName
        ##############
            selectionPaths = ['']
            ...
            f.write("<tr> <td>[[https://cmsweb.cern.ch/das/request?view=list&limit=10&instance="+dbsInst+"&input=dataset%3D"+publishName.replace("/","%2F")+"]["+publishName+"]]"
...
#rootPath = "/home/llr/cms/ndaci/WorkArea/HTauTau/Analysis/CMSSW_534p2_Spring13_PAT/src/LLRAnalysis/TauTauStudies/test/prod/tasks/HiggsSM/"
rootPath = "/home/llr/cms/ndaci/WorkArea/HTauTau/Analysis/CMSSW_534p2_AOD/src/Bianchi/TauTauStudies/test/prod/SkimAOD/SM/Dec13/"

getReportForSample(rootPath,"","HTT_SKIMAOD")
