{
  ...
  TCut lpt("ptL1>20 && (pZetaCorr-1.5*pZetaVisCorr)<999 && MtLeg1Corr>60 && MEtCorr>-999");
  TCut tpt("ptL2>20");
  TCut tiso("tightestHPSDBWP>0");
  TCut liso("combRelIsoLeg1DBeta<0.10 && tightestMVAWP>=1 && tightestAntiEMVAWP>0.5");
  ...
  backgroundDYTauTau->Draw("MtLeg1Corr>>hZTT","puWeight*sampleWeight*HLTweightTau*HLTweightElec*SFElec"*sbin);
  hZTT->Scale(4.6);
  aStack->Add(hZTT);
  if(hZTT->Integral()>0){
  ...
  backgroundWJets->Draw("MtLeg1Corr>>hW","puWeight*sampleWeight*HLTweightTau*HLTweightElec*SFElec"*sbin);
  hW->Scale(4.6);
  aStack->Add(hW);
  if(hW->Integral()>0){
  ...
  backgroundWJets->Draw("MtLeg1Corr>>hWSS","puWeight*sampleWeight*HLTweightTau*HLTweightElec*SFElec"*sbinSS);
  hWSS->Scale(4.6);
  if(hWSS->Integral()>0){
  ...
  backgroundTTbar->Draw("MtLeg1Corr>>hTT","puWeight*sampleWeight*HLTweightTau*HLTweightElec*SFElec"*sbin);
  hTT->Scale(4.6);
  aStack->Add(hTT);
  if(hTT->Integral()>0){
  ...
  backgroundOthers->Draw("MtLeg1Corr>>hOthers","puWeight*sampleWeight*HLTweightTau*HLTweightElec*SFElec"*sbin);
  hOthers->Scale(4.6);
  ...
  backgroundTTbar->Draw("MtLeg1Corr>>hTTSS","puWeight*sampleWeight*HLTweightTau*HLTweightElec*SFElec"*sbinSS);
  hTTSS->Scale(4.6);
  cout << "TT SS = " << hTTSS->Integral() << endl;

  TH1F* hVBF = new TH1F("hVBF","",nBins,xMin,xMax);
  hTT->SetFillColor(kMagenta);
  signalVBF->Draw("MtLeg1Corr>>hVBF","puWeight*sampleWeight*HLTweightTau*HLTweightElec*SFElec"*sbin);
  hVBF->SetLineWidth(3);
  hVBF->Scale(4.6*5);
  if(hVBF->Integral()>0){
  ...
  leg->SetHeader(Form("#splitline{CMS Preliminary #sqrt{s}=7 TeV}{%.1f fb^{-1} #tau_{e}#tau_{had}}", 4.6 ));
