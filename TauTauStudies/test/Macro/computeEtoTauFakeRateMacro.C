	  ...
	  )
{
  ...
  vector<float*> bin1 = simFit(tnp_,category_,cutValue_,"abseta<1.5",0.75,0.75,40,120,24,true,deltaAlpha_,deltaN_,scale_);
  vector<float*> bin2 = simFit(tnp_,category_,cutValue_,"abseta>1.5",1.90,0.4,40,120,24,true,deltaAlpha_,deltaN_,scale_);
  ...
  TH1F* hMaster = new TH1F("hMaster","CMS Preliminary 2011  #sqrt{s}=7 TeV L=186 pb^{-1}; |#eta^{probe}|; fake rate from electrons",1,0,2.3);
  ...
  string header = "";
  if(category_.find("Loose")!=string::npos) header = "Against-electron loose" ;
  if(category_.find("Medium")!=string::npos) header = "Against-electron medium" ;
  if(category_.find("Tight")!=string::npos) header = "Against-electron tight" ;
