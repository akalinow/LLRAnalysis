...
TH1* getHistogram(TFile* inputFile, const std::string& channel, const std::string& tauPtBin, const std::string& process, int massPoint, int central_or_shift)
{
  std::string histogramName = Form("%s_%s/%s%i", channel.data(), tauPtBin.data(), process.data(), massPoint);
...
//   std::string inputFilePath_unweighted = "/home/llr/cms/ivo/HTauTauAnalysis/CMSSW_5_3_11_p6_NewTauID/src/LLRAnalysis/Limits/bin/results/MuTau/Results_ABCD_AntiMuMVAMedium_AntiEleLoose_HPSMVA3oldDMwLTTight_TauOldDM_OldEleID_DatacardsTTWeights/datacards/";
  std::string inputFilePath_unweighted = "/home/llr/cms/ivo/HTauTauAnalysis/CMSSW_5_3_11_p6_NewTauID/src/LLRAnalysis/Limits/bin/results/ElecTau/Results_ABCD_AntiMu3Loose_AntiEle5Medium_HPSMVA3oldDMwLTTight_TauOldDM_OldEleID_DatacardsTTWeights/datacards/";
//   std::string inputFileName_unweighted = "muTauMSSM_diTauNSVfitMass_newNames4_NoPtWeight.root";
  std::string inputFileName_unweighted = "eTauMSSM_diTauNSVfitMass_newNames4_NoPtWeight.root";
...
//   std::string inputFilePath_weighted = "/home/llr/cms/ivo/HTauTauAnalysis/CMSSW_5_3_11_p6_NewTauID/src/LLRAnalysis/Limits/bin/results/MuTau/Results_ABCD_AntiMuMVAMedium_AntiEleLoose_HPSMVA3oldDMwLTTight_TauOldDM_OldEleID_DatacardsTTWeights/datacards/";
  std::string inputFilePath_weighted = "/home/llr/cms/ivo/HTauTauAnalysis/CMSSW_5_3_11_p6_NewTauID/src/LLRAnalysis/Limits/bin/results/ElecTau/Results_ABCD_AntiMu3Loose_AntiEle5Medium_HPSMVA3oldDMwLTTight_TauOldDM_OldEleID_DatacardsTTWeights/datacards/";
//   std::string inputFileName_weighted = "muTauMSSM_diTauNSVfitMass_newNames4_PtWeight.root";
  std::string inputFileName_weighted = "eTauMSSM_diTauNSVfitMass_newNames4_PtWeight.root";
...
//   std::string channel = "muTau";
  ...
  tauPtBins["nobtag"].push_back("nobtag_low");
  tauPtBins["nobtag"].push_back("nobtag_medium");
  tauPtBins["nobtag"].push_back("nobtag_high");
  tauPtBins["btag"].push_back("btag_low");
  tauPtBins["btag"].push_back("btag_high");
  ...
  intLumiData["eleTau"]   = 19712;
  intLumiData["muTau"]  = 19712;
  ...
  for ( vint::const_iterator massPoint = massPoints.begin();
	...
	(*outputFile) << " & ";
	TH1* histogramEventCounter_central = getHistogram(inputFile_weighted, channel, *tauPtBin, "ggH", *massPoint, kCentral);
	TH1* histogramEventCounter_up      = getHistogram(inputFile_weighted, channel, *tauPtBin, "ggH", *massPoint, kUp);
	TH1* histogramEventCounter_down    = getHistogram(inputFile_weighted, channel, *tauPtBin, "ggH", *massPoint, kDown);
	std::string key_ggH = Form("ggPhi_%i", *massPoint);
	cout<<"doing mass:category:tauptBin "<<*massPoint<<":"<<*category<<":"<<*tauPtBin<<endl;
    ...
    for ( vstring::const_iterator category = categories.begin();
      ...
      for ( vstring::const_iterator tauPtBin = tauPtBins[*category].begin();
	    tauPtBin != tauPtBins[*category].end(); ++tauPtBin ) {
	(*outputFile) << " & ";
	TH1* histogramEventCounter = getHistogram(inputFile_unweighted, channel, *tauPtBin, "bbH", *massPoint, kCentral);
