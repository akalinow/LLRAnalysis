...
cgroup.add_option("--llr", dest="llr", default=False, action="store_true",
                  help="Specify this option when running on LLR T3. [Default: False]")
cgroup.add_option("--jobdir", dest="jobdir", default="LIMITS/", type="string",
                  help="Limits directory to store jobs. [Default: LIMITS/]")
...
def lxb_submit(dirs, masses, cmd='--asymptotic', opts='') :
    ...
    for dir in dirs:
        ...
        if options.printOnly :
            print "lxb-limit.py --name {JOBNAME} --jobdir {JOBDIR} {CONDOR} {LLR} --batch-options \"{QUEUE}\" --limit-options \"{METHOD} {OPTS}\" {SYS} {DIR}".format(
                JOBNAME=jobname, JOBDIR=options.jobdir, DIR=inputs.rstrip(), QUEUE=options.queue, METHOD=cmd, OPTS=opts.rstrip(), SYS=sys, CONDOR="--condor" if options.condor else "", LLR="--llr" if options.llr else "")
        else:
            os.system("lxb-limit.py --name {JOBNAME} --jobdir {JOBDIR} {CONDOR} {LLR} --batch-options \"{QUEUE}\" --limit-options \"{METHOD} {OPTS}\" {SYS} {DIR}".format(
                JOBNAME=jobname, JOBDIR=options.jobdir, DIR=inputs.rstrip(), QUEUE=options.queue, METHOD=cmd, OPTS=opts.rstrip(), SYS=sys, CONDOR="--condor" if options.condor else "", LLR="--llr" if options.llr else ""))
            ## execute
            if not options.condor:
                os.system("echo ./{JOBDIR}/{JOBNAME}/{JOBNAME}_submit.sh".format(JOBDIR=options.jobdir, JOBNAME=jobname))
            else:
...
if options.optInject :
    ...
    if not options.calculate_injected :
        ...
        for path in paths :
            ...
            if options.printOnly :
                print "lxb-injected.py --name {NAME} --method {METHOD} --input {PATH} {LXQ} {LLR} {CONDOR} --batch-options \"{SUB}\" --toys {NJOB} --mass-points-per-job {NMASSES} --limit-options \"{OPTS}\" --injected-mass {INJECTEDMASS} {PREINJECT} {MASSES}".format(
                    NAME=jobname, METHOD=method, PATH=path, SUB=options.queue, NJOB=options.toys, NMASSES=options.nmasses, OPTS=opts,PREINJECT=preinject, INJECTEDMASS=options.injected_mass, MASSES=' '.join(dirs[path]), LXQ="--lxq" if options.lxq else "", LLR="--llr" if options.llr else "", CONDOR="--condor" if options.condor else "")
            else :
                os.system("lxb-injected.py --name {NAME} --method {METHOD} --input {PATH} {LXQ} {LLR} {CONDOR} --batch-options \"{SUB}\" --toys {NJOB} --mass-points-per-job {NMASSES} --limit-options \"{OPTS}\" --injected-mass {INJECTEDMASS} {PREINJECT} {MASSES}".format(
                    NAME=jobname, METHOD=method, PATH=path, SUB=options.queue, NJOB=options.toys, NMASSES=options.nmasses, OPTS=opts,PREINJECT=preinject, INJECTEDMASS=options.injected_mass, MASSES=' '.join(dirs[path]), LXQ="--lxq" if options.lxq else "", LLR="--llr" if options.llr else "", CONDOR="--condor" if options.condor else ""))
    else :
