...
submit.py --significance-frequentist LIMITS_$2/bin-by-bin/$1/* --llr --jobdir LIMITS_$2
cd LIMITS_$2/bin-by-bin-$1/
./bin-by-bin-$1_submit.sh
cd ../..

echo "Plot the limits."
echo "cp -r LIMITS_"$2"/bin-by-bin/"$1"/ LIMITS_"$2"/bin-by-bin/"$1"_"$3" ; plot --significance-frequentist HiggsAnalysis/HiggsToTauTau/python/layouts/limit-sm.py LIMITS_"$2"/bin-by-bin/"$1"_"$3" ; mkdir LIMITS_"$2"/bin-by-bin/"$1"_"$3"/results ; mv "$1"_"$3"_limit.* LIMITS_"$2"/bin-by-bin/"$1"_"$3"/results" > plot_$1_$3.sh
