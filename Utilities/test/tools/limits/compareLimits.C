...
channel(std::string& label){
  return (label==std::string("cmb")        ||
	  ...
	  label==std::string("et")         ||
	  label==std::string("mt")         ||
	  label==std::string("mt_moriond")        ||
	  label==std::string("mt_moriondnewsoft") ||
	  label==std::string("mt_antimu2")        ||
	  label==std::string("mt_hpsmva2")        ||
	  label==std::string("mt_post")           ||
	  label==std::string("mm")         ||
...
std::string legendEntry(const std::string& channel){
  ...
  if(channel==std::string("mt_moriond"       )) title = std::string("#mu#tau_{h} Moriond");
  if(channel==std::string("mt_moriondnewsoft")) title = std::string("#mu#tau_{h} Moriond New Soft");
  if(channel==std::string("mt_antimu2"       )) title = std::string("#mu#tau_{h} AntiMu2");
  if(channel==std::string("mt_hpsmva2"       )) title = std::string("#mu#tau_{h} HPSMVA2");
  if(channel==std::string("mt_post"          )) title = std::string("#mu#tau_{h} PostMoriond");
...
void compareLimits(const char* filename, const char* channelstr, bool expected, bool observed, const char* type, double minimum=0., double maximum=20., bool log=false, const char* label="CMS Preliminary,  H#rightarrow#tau#tau,  4.9 fb^{-1} at 7 TeV, 19.4 fb^{-1} at 8 TeV", bool legendOnRight=true, bool legendOnTop=true)
{
  ...
  colors["mt_moriond"       ] = kViolet;
  colors["mt_moriondnewsoft"] = kBlue;
  colors["mt_antimu2"       ] = kGreen;
  colors["mt_hpsmva2"       ] = kOrange;
  colors["mt_post"          ] = kRed;
